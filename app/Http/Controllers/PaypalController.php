...
class PaypalController extends AppBaseController
{
    ...
    public function success(Request $request)
    {
        ...
        try {
            ...
            if (!is_null($current_user_referral_code) && !empty($current_user_referral_code)) {
                $referral_user = DB::select('SELECT * FROM users WHERE affiliate_code = ?', [ $current_user_referral_code]);
                $referral_user = $referral_user[0];
            }
            ...
            if ($referral_user && $current_user['is_paid'] == 0) {
                ...
                $affiliateUser->is_verified = true;
